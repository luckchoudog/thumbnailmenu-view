...
public class ThumbnailAnimator {
     ...
     * 菜单关闭时的监听器
     */
    private ThumbnailMenu.OnThumbnailMenuListener onMenuCloseListener;
    ...
    public void closeMenuAnimator(TransitionLayout transitionLayout,
                                  ThumbnailMenu.OnThumbnailMenuListener onMenuCloseListener) {
    ...
    private void verticalOpenMun() {
        ...
        if (init) {
            for (int i = 0; i < tranLayoutList.size(); i++) {
                ...
                currTranLayout.setVisibility(View.VISIBLE);
            ...
            showingTranLayout.setVisibility(View.VISIBLE);
